{{--
    ==== QUÉ ES ====
    Esta es la "Plantilla Maestra" o "Layout". Es el esqueleto HTML base para todo tu sitio.
...
<head>
    ...
    {{--
        ...
        `@yield('title', 'GogoDev Cursos')`:
            - `@yield('title')`: ESTE ES EL HUECO. Aquí le decimos a Blade: "En este lugar exacto, inserta el contenido de la sección llamada 'title' que definirá la vista hija".
            - `'GogoDev Cursos'`: Este es un VALOR POR DEFECTO. Si la vista hija no define una sección 'title', se usará este texto. Es muy útil para tener un fallback.
    --}}
    <title>@yield('title', 'GogoDev Cursos')</title>
...
<body>
    ...
    {{--
        `@include('partials.navbar')`:
            - `@include`: Esta directiva le dice a Blade: "Busca el archivo `partials/navbar.blade.php` y pega todo su contenido aquí".
            - POR QUÉ: Como la barra de navegación es la misma en todas las páginas, la extraemos a un archivo parcial (`partials/navbar.blade.php`) y simplemente la incluimos aquí. Esto es reutilización de código.
    --}}
    @include('partials.navbar')
    ...
    <main class="container mt-4">
        {{--
            `@yield('content')`:
            - ESTE ES EL HUECO MÁS IMPORTANTE. Aquí es donde cada página hija (about, contact, etc.) inyectará su contenido principal y único.
